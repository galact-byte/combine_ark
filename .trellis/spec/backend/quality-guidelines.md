...
- The FULL-DELTA relative move in `SendInputMouse.click` (path ①) is load-bearing and MUST NOT be reduced to a small ±1 nudge: real-machine testing showed that replacing it with a nudge makes almost no clicks register (canvas ends near-empty). The relative move is how the game accepts/positions the click; keep `move_relative(dx, dy)` with the true delta from the previous position.
...
- Canvas geometry comes from screenshot grid-line detection (`detect_grid` / `detect_grid_rect`), not hand-typed rectangles. `viewport_seed` (centered 1280×720 model) is only the ROI seed; when detection confidence is below threshold, fall back to the seed geometry and tell the user — never silently use wrong coordinates. The seed is ~1.5 cells off at non-default UI scale, so a fallback shows as periodic missed rows/cols (a 井 pattern); a seed fallback is a detection failure, not an acceptable outcome.
- `detect_grid` uses GRID FITTING, not per-line thresholding: the canvas has faint thin lines but a very strong bold center cross + borders, so a fixed `peak*ratio` threshold drops the thin lines (only ~4 survive) and detection fails. Instead, knowing there are exactly 24 cells (25 lines), search for the (offset, pitch) whose 25-line comb maximizes total gradient energy; bold lines only help anchor the phase and missing thin lines are tolerated. Confidence = peak/baseline ratio.
- Grid detection requires an EMPTY canvas: a filled/painted canvas occludes the thin grid lines and makes detection fall back. Calibration must be done before any auto-fill.
