...
- Two-page palette is confirmed: top page shows color numbers 1–24 (indices 0–23); scrolled-to-bottom shows 17–40 (indices 16–39, offset 16 — matches `palette_center`'s `color-16`). The panel swaps content in place, so `lower_palette == palette`; use a generous `scroll_clicks` (over-scroll saturates at the bottom) rather than an exact count.
...
- [ ] Palette geometry comes from color-match detection (not ratio/canvas-anchor); lower_palette == palette; scroll_clicks over-scrolls to the bottom page.
