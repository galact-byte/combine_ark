...
## 2026-08-11 — 网格检测改网格拟合，修井字

- 真机“井字”：像素扫描确认 2整行+2整列未填。用用户提供的空白画布图跑检测：detect_grid_rect used=False——回退了种子，种子 y 偏38px。
- 检测失败因：画布细线浅、中间粗十字能量极强，0.35×峰阈值把细线滤掉只剩4条。改为网格拟合：已知24格，搜索起点+格距使25线能量最大（注意 pitch 步长要细，否则错过真实pitch陷入局部最大）。
- 真机验证：used=True, Rect(395,177,632,630) 贴合。教训：校准必须在空白画布上（已填会遮挡细线）。
- 验证：pytest 66 passed；compileall 通过。
