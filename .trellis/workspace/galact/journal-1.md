...
## 2026-08-11 — 教训：click 全量相对位移不可去

- 为减少零星漏点，把 click 里全量相对位移改小幅唤醒 → 真机几乎全空（连选色点击也不生效，故只剩最后选中的深蓝零星格，色板停在第二页）。用户疑惑是否第二页问题——不是，好结果里第二页蓝紫头发是对的。
- 已回退 click 到能用版本（全量相对位移 + PAUSE=0.03）。spec 固化：该相对位移是游戏接收/定位点击的关键，不可改小。
